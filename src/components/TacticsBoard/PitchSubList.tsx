...
import {
  getVietnameseShortName,
  getRoleBorderLeftClass,
} from "@/utils/pitchHelpers";
...
export const PitchSubList: React.FC<PitchSubListProps> = ({
  ...
  return (
    <div
      ...
      onDrop={(e) => onDropSub && onDropSub(e, slotId)}
            ...
            <div className="flex items-center space-x-0.5 shrink-0 opacity-80 group-hover/sub:opacity-100">
              {onPromoteSubToMain && (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    onPromoteSubToMain(slotId, subP.id);
                  }}
              ...
              {onClearSubPlayer && (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    onClearSubPlayer(slotId, subP.id);
                  }}
        ...
        {subPlayers.length < 5 && (
          <div
            ...
            className={`w-full py-1 border border-dashed border-slate-200 rounded text-center text-xs font-semibold text-slate-400 ${
              isFullscreen
                ? "hover:bg-blue-50/60 hover:text-blue-600 hover:border-blue-300 cursor-pointer"
                : "cursor-default"
            } transition-colors`}
          >
            {isFullscreen ? "+ Thêm dự bị" : "+ Kéo dự bị vào đây"}
          </div>
        )}
      </div>
    </div>
