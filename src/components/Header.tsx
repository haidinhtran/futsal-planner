...
export const Header: React.FC<HeaderProps> = ({ activeTab, setActiveTab, onDataRefresh }) => {
  ...
  return (
    <>
        ...
        <div className="w-full max-w-[1920px] mx-auto px-3 sm:px-4 lg:px-6">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-2.5 sm:space-x-3 shrink-0">
              <div className="w-10 h-10 sm:w-11 sm:h-11 bg-white border border-slate-300 flex items-center justify-center p-1">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm xl:text-base text-slate-500 font-semibold mt-0.5 sm:mt-1">Futsal Tactics & Squad Planner</p>
              </div>
            </div>

            {/* Desktop Navigation Tabs */}
            <nav className="hidden md:flex space-x-1 bg-slate-100 p-1 border border-slate-200">
              <button
                onClick={() => setActiveTab('tactics')}
                className={`flex items-center space-x-2 px-4 py-2 text-sm xl:text-base font-bold transition-all cursor-pointer ${
                  activeTab === 'tactics'
            ...
            {/* Secondary Action: Settings Dropdown Menu */}
            <div className="relative" ref={settingsMenuRef}>
              ...
              <button
                ...
                className={`flex items-center space-x-1.5 px-3 py-1.5 sm:px-3.5 sm:py-2 text-sm xl:text-base font-extrabold border transition-all cursor-pointer ${
                  showSettingsMenu
              ...
              >
                <Settings className={`w-4 h-4 ${showSettingsMenu ? 'text-white' : 'text-slate-600'}`} />
                <span className="hidden sm:inline">Cài đặt</span>
                <ChevronDown className={`w-3.5 h-3.5 ${showSettingsMenu ? 'text-white' : 'text-slate-500'}`} />
              </button>

              {/* Dropdown Menu Popup */}
              {showSettingsMenu && (
                <div className="absolute right-0 mt-1 w-52 bg-white border border-slate-300 py-2 z-50">
                  <div className="px-3.5 py-1 mb-1 border-b border-slate-200 text-xs font-black text-slate-400 uppercase tracking-wider">
                  ...
                  <button
                    ...
                    className="w-full flex items-center space-x-2.5 px-4 py-2 text-sm xl:text-base font-bold text-slate-700 hover:bg-blue-50 hover:text-blue-600 transition-colors text-left"
                  >
                  ...
                  <button
                    ...
                    className="w-full flex items-center space-x-2.5 px-4 py-2 text-sm xl:text-base font-bold text-red-600 hover:bg-red-50 transition-colors text-left"
                  >
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-300 px-3 py-2 flex items-center justify-around pb-safe">
        <button
          onClick={() => setActiveTab('tactics')}
          className={`flex flex-col items-center space-y-0.5 py-1 px-4 transition-all cursor-pointer ${
            activeTab === 'tactics' ? 'text-blue-600 font-black' : 'text-slate-500 font-bold hover:text-slate-900'
          }`}
        >
          <Layout className="w-5 h-5" />
          <span className="text-xs font-semibold">Thế Trận</span>
        </button>

        <button
          onClick={() => setActiveTab('players')}
          className={`flex flex-col items-center space-y-0.5 py-1 px-4 transition-all cursor-pointer ${
            activeTab === 'players' ? 'text-blue-600 font-black' : 'text-slate-500 font-bold hover:text-slate-900'
          }`}
        >
          <Users className="w-5 h-5" />
          <span className="text-xs font-semibold">Cầu Thủ</span>
        </button>

        <button
          onClick={() => setActiveTab('presentation')}
          className={`flex flex-col items-center space-y-0.5 py-1 px-4 transition-all cursor-pointer ${
            activeTab === 'presentation' ? 'text-blue-600 font-black' : 'text-slate-500 font-bold hover:text-slate-900'
          }`}
        >
          <PenTool className="w-5 h-5" />
          <span className="text-xs font-semibold">Diễn Giải</span>
        </button>
      </div>
    </>
  );
